...
export class BenefitReportComponent extends BaseComponent implements OnInit {
  ...
  private getBenefitCostPerEmployee(employee: Employee): number {
    ...
    console.log('benefit total cost(all): ' + totalCost);
    return totalCost;
  }
  ...
  private getBenefitCostForDependents(employee: Employee): number {
    ...
    dependents.forEach(dependent => {
      ...
      if (typeof dependent.birthday !== 'undefined' && dependent.birthday != null ) {
        ...
        if (typeof dependent.dentalPlanFlag !== 'undefined' && dependent.dentalPlanFlag != null 
            && dependent.dentalPlanFlag && employee.dentalPlan != null) {
          let dentalCost = this.getUnitBenefitCost('D', employee.dentalPlan.id, age);
          totalCost = totalCost + dentalCost;
        }
        if (typeof dependent.visionPlanFlag !== 'undefined' && dependent.visionPlanFlag != null 
            && dependent.visionPlanFlag && employee.visionPlan != null) {
          let visionCost = this.getUnitBenefitCost('V', employee.visionPlan.id, age);
          totalCost = totalCost + visionCost;
        }
      }
  ...
  private getUnitBenefitCost(type: string, planId: number, age: number): number {
    let benefitCost = 0;
    if (this.allPlanCosts
          .filter(pc => pc.type == type)
    ...
    console.log('Benefit Cost(' + type + '-' + planId + '-' + age + '):' + benefitCost);
    return benefitCost;
  }
